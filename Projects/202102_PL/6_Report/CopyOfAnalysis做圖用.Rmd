...
Scientificness_study_icc <- (performance::icc(m00) %>% unlist())[1]
...
#  group_by(article_type) %>%
  summarise(percent = sum(response)/n(),
            n = n(),
            yes = sum(response))
...
test_m1 <- test_m1[c("statistic","df","p.value")] %>% as.data.frame() %>% round(2)
...
test_m2a <- test_m2a[c("statistic","df","p.value")] %>% as.data.frame() %>% round(2)

test_m2b <- anova(m2b, m1) %>% broom::tidy() %>%
  filter(term == "m2b")

test_m2b <- test_m2b[c("statistic","df","p.value")] %>% as.data.frame() %>% round(2)
...
test_m3a <- test_m3a[c("statistic","df","p.value")] %>% as.data.frame() %>% round(2)

test_m3b <- anova(m3b, m1) %>% broom::tidy() %>%
  filter(term == "m3b")

test_m3b <- test_m3b[c("statistic","df","p.value")] %>% as.data.frame() %>% round(2)
...
test_m4a <- test_m4a[c("statistic","df","p.value")] %>% as.data.frame() %>% round(2)

test_m4b <- anova(m4b, m1) %>% broom::tidy() %>%
  filter(term == "m4b")

test_m4b <- test_m4b[c("statistic","df","p.value")] %>% as.data.frame() %>% round(2)
...
bind_rows(test_m1,
    ...
    mutate(p.value=if_else(p.value == 0 , "<.01",as.character(p.value)) ) %>%
...
test_m0b <- anova(m0a, m0b) %>% broom::tidy() %>%
  filter(term == "m0b")

test_m0b <- test_m0b[c("statistic","df","p.value")] %>% as.data.frame() %>% round(2)
...
test_m1a <- anova(m0a, m1a) %>% broom::tidy() %>%
  filter(term == "m1a")

test_m1a <- test_m1a[c("statistic","df","p.value")] %>% as.data.frame() %>% round(2)

test_m1b <- anova(m0a, m1b) %>% broom::tidy() %>%
  filter(term == "m1b")

test_m1b <- test_m1b[c("statistic","df","p.value")] %>% as.data.frame() %>% round(2)
...
test_m2a <- anova(m2, m2a) %>% broom::tidy() %>%
  filter(term == "m2a")

test_m2a <- test_m2a[c("statistic","df","p.value")] %>% as.data.frame() %>% round(2)

test_m2b <- anova(m2, m2b) %>% broom::tidy() %>%
  filter(term == "m2b")
...
m3b <- glmer(score ~(1 | Test_id) + NH+ OSA * ability +  OSA * familiarity + (1| jatosStudyResultId), data = df_knowledge_ext, family = binomial("logit"))

anova(m0a, m3b) 


test_m3 <- anova(m0a, m3) %>% broom::tidy() %>%
  filter(term == "m4a")

test_m3 <- test_m3[c("statistic","df","p.value")] %>% as.data.frame() %>% round(2)

test_m3b <- anova(m0a, m3b) %>% broom::tidy() %>%
  filter(term == "m4b")
...
explore_table_Knowledge <- bind_cols(
  Parameters = c(
    "Random Intercept Participant",
    "Random Slope OSA by Participant",
    "Random Slope No Subheadings by Participant",
...
`r knitr::kable(explore_table_Knowledge)`
...
test_m3a <- anova(m2a, m3a) %>% broom::tidy() %>%
  filter(term == "m3a")

test_m3a <- test_m3a[c("statistic","df","p.value")] %>% as.data.frame() %>% round(2)

test_m3b <- anova(m2a, m3b) %>% broom::tidy() %>%
  filter(term == "m3b")
...
test_m4a <- anova(m2a, m4a) %>% broom::tidy() %>%
  filter(term == "m4a")

test_m4a <- test_m4a[c("statistic","df","p.value")] %>% as.data.frame() %>% round(2)

test_m4b <- anova(m2a, m4b) %>% broom::tidy() %>%
  filter(term == "m4b")
...
explore_table_Credibility <- bind_cols(
  Parameters = c(
    "Random Intercept Participant",
    "Random Slope OSA by Participant",
    "Random Slope No Subheadings by Participant",
...
```
`r knitr::kable(explore_table_Credibility)`
...
explore_table_Veracity <- bind_cols(
  Parameters = c(
    "Random Intercept Participant",
    "Random Slope OSA by Participant",
    "Random Slope No Subheadings by Participant",
...
```
`r knitr::kable(explore_table_Veracity)`
...
explore_table_Decision <- bind_cols(
  Parameters = c(
    "Random Intercept Participant",
    "Random Slope OSA by Participant",
    "Random Slope No Subheadings by Participant",
...
`r knitr::kable(explore_table_Decision)`
...
table6 <- left_join(explore_table_Comprehensibility,explore_table_Knowledge, by = "Parameters") %>% 
  left_join(explore_table_Credibility, by = "Parameters") %>%
  left_join(explore_table_Veracity, by = "Parameters") %>%
  left_join(explore_table_Decision, by = "Parameters")

names(table6) <- c("Parameters",
                   "Comprehensibility_chisq",
                   "Comprehensibility_df",
                   "Comprehensibility_p",
                   "Credibility_chisq",
                   "Credibility_df",
                   "Credibility_p",
