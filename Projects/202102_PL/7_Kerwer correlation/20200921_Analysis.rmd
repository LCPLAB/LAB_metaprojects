...
H2 <- df_wiss %>%   
  group_by(study) %>%
  summarise(scores = sum(as.numeric(test),na.rm = TRUE)/332)

H3 <- df %>%
  group_by(study) %>%
  summarise(scores = sum(as.numeric(vtr),na.rm = TRUE))

H4 <- df %>%
  group_by(study) %>%
  summarise(scores = sum(as.numeric(wahr),na.rm = TRUE))

H5 <- df %>%
  group_by(study) %>%
  summarise(scores = sum(as.numeric(ent),na.rm = TRUE))

H2_H3_cor <- cor(H2,H3, use = "complete.obs", method = "pearson")

H2_H4_cor <- cor(H2,H4, use = "complete.obs", method = "pearson")

H2_H5_cor <- cor(H2,H5, use = "complete.obs", method = "pearson")
