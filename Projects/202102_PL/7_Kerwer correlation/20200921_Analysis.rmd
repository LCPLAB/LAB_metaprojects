...
H1_H3_cor <- cor.test(df$vst,df$vtr, use = "complete.obs", method = "pearson") %>% 
  broom::tidy()

H1_H4_cor <- cor.test(df$vst,df$wahr, use = "complete.obs", method = "pearson") %>% 
  broom::tidy()

H1_H5_cor <- cor.test(df$vst,df$ent, use = "complete.obs", method = "pearson") %>% 
  broom::tidy()
...
H2_H3_cor <- cor.test(H2$M_accquision,H2$M_credibility, use = "complete.obs", method = "pearson") %>% 
  broom::tidy()


H3 <- df %>%
  group_by(id) %>%
  summarise(score2 = sum(as.numeric(vtr),na.rm = TRUE)/12) #每位參與者的回答平均

H4 <- df %>%
  group_by(id) %>%
  summarise(score2 = sum(as.numeric(wahr),na.rm = TRUE)/12)

H5 <- df %>%
  group_by(id) %>%
  summarise(score2 = sum(as.numeric(ent),na.rm = TRUE)/12)

H2_H3_cor <- cor(H2$score1,df$vtr, use = "complete.obs", method = "pearson")

H2_H4_cor <- cor(H2$score1,H4$score2, use = "complete.obs", method = "pearson")

H2_H5_cor <- cor(H2$score1,H5$score2, use = "complete.obs", method = "pearson")
...
ggscatter(H23, x = "score1", y = "score2",
...
ggscatter(H24, x = "score1", y = "score2",
   add = "reg.line",  # Add regressin line
   add.params = list(color = "blue", fill = "lightgray"), # Customize reg. line
   conf.int = TRUE # Add confidence interval
   )

ggscatter(H25, x = "score1", y = "score2",
   add = "reg.line",  # Add regressin line
   add.params = list(color = "blue", fill = "lightgray"), # Customize reg. line
   conf.int = TRUE # Add confidence interval
   )
