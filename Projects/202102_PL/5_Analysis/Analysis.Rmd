...
## which((colSums(df_Basic!="None",na.rm = TRUE) != dim(df_Basic)[1]) |
...
## Exclude the variables full of "None"
df_Post <- df_Post[,which(colSums(df_Post!="None",na.rm = TRUE) != 0)]


## Split knowledge data
df_knowledge <- filter(df, Q_type == "Knowledge" & Q_presented == 1)
## Exclude the variables full of "None"

df_knowledge <- left_join(select(df_knowledge[,which(colSums(df_knowledge!="None",na.rm = TRUE) != 0)], -c("NH","OSA","text_order")),
  select(df_Post, jatosStudyResultId, NH, OSA, Text_id, text_order) %>% distinct() %>% mutate(Text_id = as.character(Text_id)),## Summarise the text order by participant
by=c("jatosStudyResultId","Test_id"="Text_id"))
