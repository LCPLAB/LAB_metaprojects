...
##將包含中文的response替換數字
tb_Post <- (dm_Post <- df_Post %>% filter(Topic %in% c("Comprehensibility","Scientificness","Credibility","Veracity","Decision") & !(jatosStudyResultId %in% strange_id)) %>%
...
dm_Comprehense <- subset(dm_Post, Topic == "Comprehensibility")

dm_Comprehense$response = as.numeric(dm_Comprehense$response)

# conduct icc

m0 <- lmer(response ~ (1 | jatosStudyResultId), data = dm_Comprehense )

summary(m0)
performance::icc(m0)

m00 <- lmer(response ~ (1 | Test_id), data = dm_Comprehense)
...
m1 <- lmer(response ~ (1 | jatosStudyResultId) + article_type + (1 | Test_id), data = dm_Comprehense)
...
```{r knowledge-stat}
# mixed-effect models
m0 <- glmer(score ~ (1 | jatosStudyResultId), family = binomial("logit"), data = dm_knowledge)

m00 <- glmer(score ~ (1 | Test_id), family = binomial("logit"), data = dm_knowledge)


m1 <- glmer(score ~ (1 | jatosStudyResultId) + article_type + (1 | Test_id), family = binomial("logit"), data = dm_knowledge)
